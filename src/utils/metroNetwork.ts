...
class Network {
	...
	distribute() {
		const latRange = [-Infinity, Infinity];
		const lngRange = [-Infinity, Infinity];
	...
	getIntersections(): Map<string, Set<Station>> {
		...
		[...this.stations.values()]
			.filter((station) => station.lines.size > 1)
			.forEach((station) => {
	...
	getPath(startStationName: StationName, endStationName: StationName): [Station[], string[]] {
		...
		// @ts-expect-error intersection method exists on Set
		const commonLines = startStation.lines.intersection(endStation.lines) as Set<Line>;
		...
		if (commonLinesArray.length > 0) {
			...
			const instructions = [`Take  ${direction?.name} direction, ${commonLinesArray[0].name}`, `Ride ${path.length} stations.`];
			...
			for (const eLine of endStation.lines || []) {
				...
				Array.from(results as Set<Station>).forEach((intersection) => {
					...
					if (length < currentLength) {
						...
						instructions = [`Ride ${path1.length} stations on ${direction1?.name} direction, ${sLine.name}.`, `Switch to ${eLine.name} on ${intersection?.name}.`, `Ride ${path2.length} stations on ${direction2?.name} direction.`];
					}
				});
			}
		}
